...
If variables are removed from the variables display, you can display them without a sign; e.g., `v bui` will make solidity builtins variables to be displayed.  You can also add these variables back in to the display with `+`; e.g., `v +bui` will make it so that solidity builtins variables will always be displayed when you enter `v`, and `v -bui` will turn this off.  All of these options can again be combined.
